...
public class SwipeItemLayout extends FrameLayout {
    ...
    private ShowModel mShowModel = ShowModel.Pull;
    ...
    private void initAttr(int attr, TypedArray typedArray) {
        if (attr == R.styleable.SwipeItemLayout_swipeDirection) {
            // 默认向左滑动
        ...
        } else if (attr == R.styleable.SwipeItemLayout_showMode) {
            // 默认是拉出
            int pullOutBottomMode = typedArray.getInt(attr, mShowModel.ordinal());

            if (pullOutBottomMode == ShowModel.BeCover.ordinal()) {
                mShowModel = ShowModel.BeCover;
            }
        } else if (attr == R.styleable.SwipeItemLayout_springDistance) {
            // 弹簧距离，不能小于0，默认值为0
        ...
        }else if(attr==R.styleable.SwipeItemLayout_enableSlide){
            mEnableSlide =typedArray.getBoolean(attr,true);
        }else if(attr==R.styleable.SwipeItemLayout_enableAlpha){
            mEnableAlpha=typedArray.getBoolean(attr,false);
        }
    }
    ...
    protected void onFinishInflate() {
        ...
        if (getChildCount() != 2) {
            throw new RuntimeException(SwipeItemLayout.class.getSimpleName() + "必须有且只有两个子控件");
        }
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        ...
        if (mSwipeDirection == SwipeDirection.Left) {
            ...
            if (mShowModel == ShowModel.BeCover) {
            ...
            switch (state) {
                ...
                case ViewDragHelper.STATE_IDLE:
                    if (mTopView.getLeft() == getPaddingLeft() + mTopLp.leftMargin) {
                        ...
                        mCurrentStatus = Status.Opened;
                        if (mIsNeedNotify && mDelegate != null && mPreStatus != mCurrentStatus) {
                            mDelegate.onSwipeItemLayoutOpened(SwipeItemLayout.this);
                        }
                    }
                    mPreStatus = mCurrentStatus;
                    mIsNeedNotify = false;

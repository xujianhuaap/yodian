...
public class HXNotifier {
    ...
    protected void sendNotification(EMMessage message, boolean isForeground, boolean numIncrease) {
        boolean push = SettingsActivity.isPush(appContext);
        if (push) {
            ...
            try {
                ...
                Bitmap icon = BitmapFactory.decodeResource(appContext.getResources(), R.mipmap.icon_app);
    ...
    public void viberateAndPlayTone(EMMessage message) {
        if (SettingsActivity.isPush(appContext)) {
            if (message != null) {
                if (EMChatManager.getInstance().isSlientMessage(message)) {
                    return;
                }
